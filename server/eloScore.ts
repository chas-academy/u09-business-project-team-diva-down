...
function getPoints(scorePercentage: number, difficulty: Difficulty, currentRating: number): number {
    ...
    for (const [threshold, points] of pointsBracket) {
        if (scorePercentage >= threshold) {
            const finaleScore: number =  Math.round(points * scale);
...
function multiplayerRating(currentRatingOfPlayer: number, difficulty: Difficulty, wins: number): number {
    const difficultyScale: Record<Difficulty, number> = {
        easy: 0.50,
        medium: 0.75,
        hard: 1.0,
...
function checkUserBracket(rating: number): string {
